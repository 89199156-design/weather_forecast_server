...
GFS and CAMS WebP layers are generated from local Open-Meteo `.om` runtime data
through the unmodified Open-Meteo HTTP API. The reusable layer encoder is
`scripts/build_webp.py`: it requests grid-point batches from the local official
API container, then renders WebP from those API values.
...
the GFS production cycle download source data, restart the local official
Open-Meteo API container, and rebuild the GFS WebP layers through that API.

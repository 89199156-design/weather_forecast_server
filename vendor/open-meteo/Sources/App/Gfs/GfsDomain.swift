...
enum GfsDomain: String, GenericDomain, CaseIterable {
    ...
    func getGribUrl(run: Timestamp, forecastHour: Int, member: Int, useAws: Bool) -> [String] {
        ...
        /// 4 week archive
        let gfsAws = WeatherForecastServerSourceConfig.baseUrl(
            "WEATHER_GFS_AWS_BASE_URL",
            fallback: "https://noaa-gfs-bdp-pds.s3.amazonaws.com/"
        )

        let gfsNomads = WeatherForecastServerSourceConfig.baseUrl(
        ...
        let gefsAws = WeatherForecastServerSourceConfig.baseUrl(
            "WEATHER_GEFS_AWS_BASE_URL",
            fallback: "https://noaa-gefs-pds.s3.amazonaws.com/"
        )
        let gefsNomads = WeatherForecastServerSourceConfig.baseUrl(
            "WEATHER_GEFS_NOMADS_BASE_URL",
            fallback: "https://nomads.ncep.noaa.gov/pub/data/nccf/com/gens/prod/"
        )
        let gefsServer = useArchive ? gefsAws : gefsNomads

        let hrrrNomads = WeatherForecastServerSourceConfig.baseUrl(
            "WEATHER_HRRR_NOMADS_BASE_URL",
            fallback: "https://nomads.ncep.noaa.gov/pub/data/nccf/com/hrrr/prod/"
        )
        let hrrrAws = WeatherForecastServerSourceConfig.baseUrl(
            "WEATHER_HRRR_AWS_BASE_URL",
            fallback: "https://noaa-hrrr-bdp-pds.s3.amazonaws.com/"
        )
        ...
        case .gfswave025_ens:
         ...
         case .nam_conus:
            let namNomads = WeatherForecastServerSourceConfig.baseUrl(
                "WEATHER_NAM_NOMADS_BASE_URL",
                fallback: "https://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/"
            )
            return ["\(namNomads)nam.\(run.format_YYYYMMdd)/nam.t\(run.hh)z.conusnest.hiresf\(fHH).tm00.grib2"]
        case .hrrr_conus:

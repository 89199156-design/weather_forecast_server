...

{
  ...
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS] download runtime data run=$RUN"
  ...
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS] restart local Open-Meteo API"
  bash scripts/deploy_singapore_candidate.sh

  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS] build CAMS layer products"

...
if [[ -z "$RUN" ]]; then
  printf '%s\n' "Usage: run_cams_production_cycle.sh YYYYMMDDHH" >&2
  exit 2
fi

APP_DIR="${WEATHER_FORECAST_APP_DIR:-/opt/1panel/apps/weather_forecast_server}"
LOG_DIR="${WEATHER_OPENMETEO_BUILD_LOG_DIR:-/opt/1panel/apps/weather/logs}"
LOCK_FILE="${WEATHER_OPENMETEO_CAMS_LOCK_FILE:-/tmp/weather_openmeteo_cams_cycle.lock}"
...
{
  flock -n 9 || {
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS] previous job still running, skip."
    exit 0
  }
  ...
  echo "$download_start [OPENMETEO_CAMS] download runtime data run=$RUN start=$download_start"
  bash scripts/download_openmeteo_cams_data.sh
  download_end="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "$download_end [OPENMETEO_CAMS] download runtime data run=$RUN end=$download_end"
  ...
  echo "$layer_start [OPENMETEO_CAMS] build CAMS layer products start=$layer_start"
  bash scripts/build_openmeteo_cams_layers.sh
  layer_end="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "$layer_end [OPENMETEO_CAMS] build CAMS layer products end=$layer_end"

  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS] completed run=$RUN"
} 9>"$LOCK_FILE" >> "$LOG_DIR/openmeteo_cams_cycle.log" 2>&1

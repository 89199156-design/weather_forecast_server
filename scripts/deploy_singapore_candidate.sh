...
capture_weather_env_overrides() {
  ...
  while IFS='=' read -r name _; do
    if [[ "$name" == WEATHER_* || "$name" == "DATA_RUN_DIRECTORY" || "$name" == "CACHE_FILE" || "$name" == "CACHE_SIZE" || "$name" == "BLOCK_SIZE" || "$name" == "CACHE_META_FILE" || "$name" == "CACHE_META_SIZE" ]]; then
...
require_dem_source() {
  ...
  printf '%s\n' \
    "Missing Copernicus DEM90 source. Pre-seed $DATA_DIR/copernicus_dem90/static/lat_*.om before deploying." >&2
  exit 2
}
...
env | sort | awk -F= '
  ($1 ~ /^WEATHER_/ || $1 == "DATA_RUN_DIRECTORY" || $1 == "CACHE_FILE" || $1 == "CACHE_SIZE" || $1 == "BLOCK_SIZE" || $1 == "CACHE_META_FILE" || $1 == "CACHE_META_SIZE") && $2 != "" { print }
' > "$SANITIZED_ENV_FILE"

...
TASK_NAME="weather_cams_ecpds_probe_cycle"
...
finish_task() {
  ...
  trap - EXIT
  set +e
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] stage=cleanup after task rc=$main_rc"
  if container_state="$(openmeteo_task_container_state "$TASK_SCOPE")"; then
    if [[ "$container_state" == "running" ]]; then
      echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] preserve running task container and staging during final cleanup"
      ...
      python3 "$APP_DIR/scripts/cleanup_native_task_staging.py" \
        ...
        --scope cams_ecpds || cleanup_rc=$?
    else
      cleanup_rc=$?
      echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] preserve staging because container cleanup failed rc=$cleanup_rc" >&2
    fi
  else
    cleanup_rc=$?
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] preserve container and staging because state inspection failed rc=$cleanup_rc" >&2
  fi
  final_rc=$main_rc
  if (( cleanup_rc != 0 )); then
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] final cleanup failed rc=$cleanup_rc" >&2
    if (( final_rc == 0 )); then
      final_rc=$cleanup_rc
    fi
  fi
...
trap finish_task EXIT
{
  ...
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP_SCHEDULE] stage=startup cleanup previous abnormal task residue"

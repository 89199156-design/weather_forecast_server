...

{
  ...
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP] download runtime data run=$RUN"
  ...
  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP] restart local Open-Meteo API"
  bash scripts/deploy_singapore_candidate.sh

  echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [OPENMETEO_CAMS_FTP] build CAMS layer products"

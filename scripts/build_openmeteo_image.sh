...
if git -C "$REPO_ROOT" rev-parse HEAD >/dev/null 2>&1; then
  # A Swift producer image depends only on the Dockerfile and vendored
  # Open-Meteo tree. Rust/API-only commits must not force a new image.
  SOURCE_ID="$({
    git -C "$REPO_ROOT" rev-parse HEAD:docker/openmeteo-engine.Dockerfile
    git -C "$REPO_ROOT" rev-parse HEAD:vendor/open-meteo
...
BUILD_ARG_ARGS=(--build-arg "SWIFT_SOURCE_ID=${SOURCE_ID:-unknown}")
